...
velocities_delta = ti.Vector.field(dim, float)
    ...
    for i in positions:
        ...
        for j in range(particle_num_neighbors[i]):
            ...
            if p_j < 0:
                break
            pos_ji = pos_i - positions[p_j]
            omegas[i] += mass * (velocities[p_j] - velocities[i]).cross(spiky_gradient(pos_ji, h)) / (epsilon + density[p_j])
...
def apply_viscosity(XSPH_C: ti.f32):
    ...
    for i in positions:
        pos_i = positions[i]
        velocities_delta[i] = pos_i * 0.0
        for j in range(particle_num_neighbors[i]):
            p_j = particle_neighbors[i, j]
            if p_j < 0:
                break
            pos_ji = pos_i - positions[p_j]
            velocities_delta[i] += mass * (velocities[p_j] - velocities[i]) * poly6_value(pos_ji.norm(), h) / (epsilon + density[p_j]) # (velocities[p_j] - velocities[i]) * poly6_value(pos_ji.norm(), h)

    for i in positions:
        velocities[i] += XSPH_C * velocities_delta[i]
