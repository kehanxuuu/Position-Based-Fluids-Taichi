...
ti.root.dense(ti.i, num_particles).place(old_positions, positions, velocities, density)
...
def substep():
    ...
    for p_i in positions:
        ...
        # grad_i += spiky_gradient(pos_i * 0.0, h)
    ...
    for p_i in positions:
        ...
        # scorr_ii = compute_scorr(pos_i * 0.0)
        # pos_delta_i += (lambda_i + lambda_i + scorr_ii) * spiky_gradient(pos_i * 0.0, h)  # self contribution
...
def compute_density():
    for p_i in positions:
        ...
        for j in range(particle_num_neighbors[p_i]):
            ...
            pos_ji = pos_i - positions[p_j]
            # Eq(2)
            density_i += poly6_value(pos_ji.norm(), h)  # mass in Eq(2) is moved to Eq(1)

        # Eq(1)
        density_i += poly6_value(0, h)  # self contribution
        density[p_i] = density_i * mass
...
def render(gui):
    ...
    density_np = (density_np - rho0) / (4.0 * rho0) + 0.5  # map to [0, 1]
    colors = cm.RdBu(density_np)[:, :3]
    # v_rel = np.linalg.norm(velocities.to_numpy(), axis=1) / 15.0
    # colors = cm.jet(v_rel)[:, :3]
    gui.circles(pos_np, radius=particle_radius, color=np.array([ti.rgb_to_hex(x) for x in colors]))
